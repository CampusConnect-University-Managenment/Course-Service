...
public class CourseAdminController {
    ...
    @GetMapping("/filter")
    public ResponseEntity<CommonResponse<List<Course>>> getCoursesByFilters(
            @RequestParam String department,
            @RequestParam String year,
            @RequestParam String semester) {

        List<Course> filteredCourses = courseService.getCoursesForStudent(department, year, semester);
        return ResponseEntity.ok(
                new CommonResponse<>(true, "Filtered courses retrieved successfully", filteredCourses)
        );
    }
